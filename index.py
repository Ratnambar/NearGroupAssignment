...
# connecting Flask App with Mongo database(used 'test' as database)
...
class TodoList(Resource):
    def get(self):      ## for fetching all data from crudApp collections.
        customers = mongo.db.crudApp.find()
        resp = dumps(customers)
        res = resp.replace("\"","")
        return res

    ## creating new data into collection 'crudApp'.
    def post(self):
...
class TodoAgain(Resource):
    ## get perticular record from collection.
    def get(self,id):
